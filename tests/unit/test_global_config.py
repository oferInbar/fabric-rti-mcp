...
from defender_ah_mcp.config import GlobalDefenderAHConfig
...
class TestFromEnvBooleanParsing:
    ...
    def test_obo_flow_true_string(self) -> None:
        config = GlobalDefenderAHConfig.from_env()
    ...
    @patch.dict("os.environ", {"DEFENDER_AH_STATELESS_HTTP": "true"}, clear=False)
    def test_stateless_http_true(self) -> None:
        config = GlobalDefenderAHConfig.from_env()
        assert config.stateless_http is True

    @patch.dict("os.environ", {"DEFENDER_AH_STATELESS_HTTP": "false"}, clear=False)
    def test_stateless_http_false(self) -> None:
        config = GlobalDefenderAHConfig.from_env()
        assert config.stateless_http is False

    @patch.dict("os.environ", {"DEFENDER_AH_AI_FOUNDRY_COMPATIBILITY_SCHEMA": "true"}, clear=False)
    def test_ai_foundry_compat_true(self) -> None:
        config = GlobalDefenderAHConfig.from_env()
        assert config.use_ai_foundry_compat is True

    @patch.dict("os.environ", {"DEFENDER_AH_AI_FOUNDRY_COMPATIBILITY_SCHEMA": "false"}, clear=False)
    ...
    def test_cors_allowed_origins_defaults_to_wildcard(self) -> None:
        ...
        os.environ.pop("DEFENDER_AH_CORS_ORIGINS", None)
        config = GlobalDefenderAHConfig.from_env()
        assert config.cors_allowed_origins == "*"

    @patch.dict("os.environ", {"DEFENDER_AH_CORS_ORIGINS": "https://example.com,https://other.com"}, clear=False)
...
class TestWithArgsCLIOverride:
    ...
    def test_env_obo_respected_without_cli_flag(self) -> None:
        ...
        try:
            sys.argv = ["prog"]
            config = GlobalDefenderAHConfig.with_args()
            assert config.use_obo_flow is True
        finally:
...
class TestAhModeParsing:
    ...
    def test_empty_string_means_disabled(self) -> None:
        assert GlobalDefenderAHConfig._parse_ah_mode("") == ""

    def test_advanced_hunting_value(self) -> None:
        assert GlobalDefenderAHConfig._parse_ah_mode("AdvancedHunting") == "AdvancedHunting"

    def test_vibe_hunting_value(self) -> None:
        assert GlobalDefenderAHConfig._parse_ah_mode("VibeHunting") == "VibeHunting"
    ...
    def test_legacy_true_maps_to_vibe_hunting(self) -> None:
        assert GlobalDefenderAHConfig._parse_ah_mode("true") == "VibeHunting"
        assert GlobalDefenderAHConfig._parse_ah_mode("True") == "VibeHunting"
        assert GlobalDefenderAHConfig._parse_ah_mode("1") == "VibeHunting"

    def test_unknown_value_defaults_to_empty(self) -> None:
        assert GlobalDefenderAHConfig._parse_ah_mode("InvalidMode") == ""
