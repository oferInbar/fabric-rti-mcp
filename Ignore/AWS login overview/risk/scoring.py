...
def heuristic_score(logins: pd.DataFrame, baseline: pd.DataFrame) -> pd.DataFrame:
    """
    Returns a DataFrame with one row per User and the columns:
    User, HeuristicScore, ImpossibleTravel, NewCountries, NewIps, Countries, Ips, Logins.
    """
    if logins.empty:
        return pd.DataFrame(
            columns=[
    ...
    for user, g in logins.groupby("User"):
        ...
        # Normalize: 1 new country = 0.5, 2+ = 1.0
        r_country = min(1.0, len(new_countries) * 0.5) if b_countries else 0.0
        # New IPs: log-ish curve, cap at 1.0
...
def alert_score(alerts: pd.DataFrame) -> pd.DataFrame:
    """Severity-weighted alert score in [0,1] per user."""
    if alerts.empty:
...
def combine_scores(
    heuristic: pd.DataFrame, alerts: pd.DataFrame
) -> pd.DataFrame:
    """Soft-max: per-user risk = max(heuristic, alert)."""
    if heuristic.empty:
        return heuristic
